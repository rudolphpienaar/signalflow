...
Usage:
  global_config_load()   — called once at CLI startup
  global_config_reset()  — called by diagram_render before per-doc config,
...
# Empty dict means global_config_load() was never called (e.g. in tests),
# in which case global_config_reset() is a safe no-op.
_baseline: dict = {}
...
def global_config_load() -> None:
    """Load user and project configs, then capture the result as the baseline.

    Call this once at CLI startup.  After this call, global_config_reset()
...
def global_config_reset() -> None:
    """Reset the config singleton to the startup baseline.

    No-op if global_config_load() was never called (e.g. in tests).
