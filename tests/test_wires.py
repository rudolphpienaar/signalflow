...
class TestForwardWire:
    def test_forward_wire_places_entry_glyph(self):
        ...
        # Bind port so layout maps entry rows
        child.input_ports[id(root)] = Port("sig", "ret")
    ...
    def test_signal_label_present(self):
        ...
        child = Node(module="M", func="child()", children=[])
        root  = Node(module="M", func="root()", children=[child])
        child.input_ports[id(root)] = Port("mySignal", None)
        root.output_ports[id(child)] = Port("mySignal", None)
        canvas, nodes, ow = _full_render(root)
...
class TestReturnWire:
    def test_return_wire_connects(self):
        ...
        child = Node(module="M", func="child()", children=[])
        root  = Node(module="M", func="root()", children=[child])
        child.input_ports[id(root)] = Port(None, None)
        root.output_ports[id(child)] = Port(None, None)
        canvas, nodes, ow = _full_render(root)
        r = nodes[0]
        rx = r.x + r.ow - 1
...
class TestRoutingInvariants:
    ...
    def test_double_helix_invariant(self):
        ...
        c1.input_ports[id(root)] = Port(None, None)
        c2.input_ports[id(root)] = Port(None, None)
        root.output_ports[id(c1)] = Port(None, None)
        root.output_ports[id(c2)] = Port(None, None)
