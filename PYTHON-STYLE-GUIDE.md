...
## Linting: Mandatory Pre-Suggestion Check

**`ruff` must be clean before any code is suggested or submitted.**

This project uses [ruff](https://docs.astral.sh/ruff/) as the sole linter. Run it against every file you modify or create:

```bash
ruff check src/signalflow/          # lint the full package
ruff check <path/to/changed_file.py>  # lint a specific file
```

### Rules

- **Zero tolerance**: no warnings, no errors. All `ruff` output must be empty.
- **No suppressions** (`# noqa`, `# type: ignore`) without an explicit, documented justification in the same comment.
- Fix the root cause; do not silence the linter.
- Run `ruff check --fix` to auto-resolve safe violations, then re-check manually.

### Configuration

`ruff` is not yet configured in `pyproject.toml`; it runs with its defaults. When a `[tool.ruff]` section is added, defer to it.

### Claude Code Directive

> **Every code suggestion or change I (Claude) make must be verified against `ruff check` before being presented. If the linter reports any issue, I must resolve it first. I will show the clean `ruff` output as confirmation.**
